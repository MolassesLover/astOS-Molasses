...
def add_node_to_parent(tree, id, val):
    par = anytree.find(
        tree, filter_=lambda node: ("x" + str(node.name) + "x") in ("x" + str(id) + "x")
    )
...
def add_node_to_level(tree, id, val):
    ...
    par = anytree.find(
        tree,
        filter_=lambda node: ("x" + str(node.name) + "x") in ("x" + str(npar) + "x"),
    )
    add = anytree.Node(val, parent=par)
...
def get_part():
    ...
    part = str(
        subprocess.check_output(
            f"blkid | grep '{uuid}' | awk '{{print $1}}'", shell=True
        )
    )
...
def get_tmp():
    mount = str(
        subprocess.check_output("cat /proc/mounts | grep ' / btrfs'", shell=True)
    )
    if "tmp0" in mount:
        return "tmp0"
    else:
...
def clone_recursive(snapshot):
    if not (os.path.exists(f"/.snapshots/rootfs/snapshot-{snapshot}")):
        ...
        for child in ch:
            i = clone_under(
                new_children[children.index(get_parent(fstree, child))], child
            )
    ...
    if not (os.path.exists(f"/.snapshots/rootfs/snapshot-{treename}")):
        ...
        while True:
            ...
            if os.path.exists(f"/.snapshots/rootfs/snapshot-chr{sarg}"):
                snapshot = sarg
                print(
                    f"F: snapshot {snapshot} appears to be in use. If you're certain it's not in use clear lock with 'ast unlock {snapshot}'."
                )
    ...
    if not (os.path.exists(f"/.snapshots/rootfs/snapshot-{treename}")):
        ...
        while True:
            ...
            if os.path.exists(f"/.snapshots/rootfs/snapshot-chr{sarg}"):
                ...
                pkg_list_from = str(
                    subprocess.check_output(
                        f"chroot /.snapshots/rootfs/snapshot-{arg} pacman -Qq",
                        shell=True,
                    )
                )[2:][:-1].split("\\n")[:-1]
...
def chroot(snapshot):
    if not (os.path.exists(f"/.snapshots/rootfs/snapshot-{snapshot}")):
        ...
        f"/.snapshots/rootfs/snapshot-chr{snapshot}"
    ):  # Make sure snapshot is not in use by another ast process
        print(
            f"F: snapshot {snapshot} appears to be in use. If you're certain it's not in use clear lock with 'ast unlock {snapshot}'."
        )
    else:
...
def live_install(pkg, is_aur):
    ...
    if is_aur and not aur:
        print(
            "F: AUR is not enabled in current live snapshot, but is enabled in target.\nEnable AUR for live snapshot? (y/n)"
        )
        reply = input("> ")
...
def install_profile(snapshot, profile):
    install(
        snapshot,
        subprocess.check_output(f"cat {profile}", shell=True).decode("utf-8").strip(),
    )
...
def ast_help():
    ...
    print(
        "\tlive-chroot - open a read-write shell inside currently booted snapshot (changes are discarded on new deployment)"
    )
    print("\trun <snapshot> <command> - execute command inside another snapshot")
    print(
        "\ttree-run <tree> <command> - execute command inside another snapshot and all snapshots below it"
    )
    ...
    print(
        "\tinstall <snapshot> <package> - install a package inside specified snapshot"
    )
    print(
        "\tsync <tree> - sync package and configuration changes recursively, requires an internet connection"
    )
    print(
        "\tforce-sync <tree> - same thing as sync but doesn't update snapshots, potentially riskier"
    )
...
def findnew():
    ...
    while True:
        i += 1
        if (
            str(f"snapshot-{i}") not in snapshots
            and str(f"etc-{i}") not in snapshots
            and str(f"var-{i}") not in snapshots
            and str(f"boot-{i}") not in snapshots
        ):
            return i
...
def main(args):
    ...
    fstree = importer.import_(
        import_tree_file("/.snapshots/ast/fstree")
    )  # Import fstree file
    ...
    if isChroot == True and ("--chroot" not in args):
        ...
        if args_2[0] == "--live":
            args_2.remove(args_2[0])
        if (
            args_2[0] == get_snapshot()
        ):  # If installing into current snapshot automatically use live install
            live = True
        ...
        if (
            live and not excode
        ):  # only perform the live_install if the first install was successful
